    ...
    <div className="m-2">
      <div 
        ...
        onClick={()=>navigate(`/pin-detail/${_id}`)}>
          ...
          <img className="rounded-lg w-full" alt="user-post" src={urlFor(image).width(250).forceDownload()}/>
          ...
          {postHovered && (
            <div className="absolute top-0 w-full h-full flex flex-col justify-between p-1 pr-2 pt-2 pb-2 z-50" style={{height:'100%'}}>
              <div className="flex items-center justify-between">
                ...
                {alreadySaved?(
                  <button type="button" onClick={(e)=>{e.stopPropagation()}} className="bg-red-500 opacity-70 hover:opacity-100 text-white font-bold px-5 py-1 text-base rounded-3xl hover:shadow-md outline-none">{save.length} Saved</button>
                ):(
                  <button 
                    onClick={(e)=>{
                      e.stopPropagation();
      ...
      <Link to={`user-profile/${postedBy?._id}`} className="flex gap-2 mt-2 items-center" >
        <img 
          ...
          alt="user-profile"
          />
        <p className="font-black capitalize">{postedBy?.userName}</p>
      </Link>
    </div>
  )
}
